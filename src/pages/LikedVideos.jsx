...
function LikedVideos() {
  ...
  const { data, isLoading, error, isError, refetch } = useGetUserLikedVideos(
    user?._id,
  );
      ...
      {isAuthenticated && (
        <>
          <Skeleton loading={isLoading}>
            <div
              className="relative  p-6 overflow-hidden lg:rounded-t-2xl lg:h-[calc(100vh-122px)] lg:sticky lg:top-[88px] md:px-6"
              style={{
              ...
              <div className="relative z-10 flex flex-col w-full gap-6 text-xs md:flex-row md:items-center lg:flex-col lg:w-80">
                ...
                <div className="flex flex-col w-full gap-6 ">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-medium capitalize drop-shadow-lg">
                      {user?.fullName}
                    </p>
          ...
          <div className="flex flex-col flex-1 py-4 sm:px-2 lg:py-0">
            ...
            {isLoading &&
              Array.from({ length: 3 })
                .fill(1)
                .map((item, i) => <VideoCard2 key={i} loading={isLoading} />)}
            ...
            <hr
              hidden={data?.data.length === 0 || isLoading}
