  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-1 pt-16">
        ...
        {showMenu && !isDashboardRoute && !isVideoRoute && (
          <div
            onClick={() => toggleMenu()}
            className="fixed md:hidden inset-0  bg-[--color-overlay] z-[90]"
          ></div>
        )}

        {openDashboardSidebar && isDashboardRoute && !isVideoRoute && (
          <div
            onClick={() => toggleDashboardSidebar()}
            className="fixed lg:hidden inset-0  bg-[--color-overlay] z-[90]"
          ></div>
        )}

        <Outlet
          context={[showMenu, openDashboardSidebar, toggleDashboardSidebar]}
        />
      </div>
