...
import {
  ...
  useUpdatePlaylist,
} from "../lib/queries/playlistQueries";
...
function PlaylistVideos() {
  ...
  const { mutate: deletePlaylist, isPending: isDeletingPlaylist } =
    useDeletePlaylist(playlistId, user?._id);
  ...
  const { contentRef, isExpanded, isLongContent } = useReadMore(
    ...
    <div className="flex flex-col w-full mb-16 sm:mb-0 lg:flex-row lg:p-6 mx-auto max-w-[2560px]">
      <Skeleton loading={loadingPlaylistData}>
        <div
          ...
          style={{
            backgroundImage: `url(${playlist?.data?.videos[0]?.thumbnail})`,
          }}
        >
          ...
          <div className="relative z-10 flex flex-col w-full gap-6 text-xs md:flex-row md:items-center lg:flex-col lg:w-80">
            ...
            <div className="flex flex-col w-full gap-4 ">
              ...
              <div>
                <Link
                  to={`/channel/${playlist?.data?.owner._id}/videos`}
                  className="flex items-center gap-2 my-2 hover:opacity-90"
                >
                  <Avatar
                    radius="full"
                    size={"1"}
                ...
                <Text as="p" className="drop-shadow-lg">
                  Playlist • {playlist?.data?.totalVideos}{" "}
                  {playlist?.data?.totalVideos === 1 ? "video" : "videos"} •{" "}
                  {playlist?.data?.updatedAt &&
                    `Updated ${formatDistanceToNow(
                      new Date(playlist?.data?.updatedAt),
                      {
                        addSuffix: true,
                      },
                    )}`}
              ...
              <Dialog.Root>
                <Dialog.Trigger className="cursor-pointer">
                  <Text as="div" tabIndex={"0"} className="relative">
                    ...
                    {isLongContent && (
                      <button className="absolute w-full font-semibold text-left drop-shadow-lg">
                        ...more
                      </button>
                    )}
                  </Text>
                        ...
                        <label>
                          ...
                          <TextField.Root
                            ...
                            {...register("name", {
                              ...
                              validate: (value) =>
                                value.trim() !== "" || "Please enter some text",
                            })}
                  ...
                  <AlertDialog.Root>
                    ...
                    <AlertDialog.Content maxWidth="450px">
                      ...
                      <AlertDialog.Description size="2">
                        Are you sure ? Deleting playlists is a permanent action
                        and cannot be undone.
                      </AlertDialog.Description>

                      <Flex gap="3" mt="4" justify="end">
                        <AlertDialog.Cancel disabled={isDeletingPlaylist}>
                          <Button color="gray" radius="full" variant="surface">
                            Cancel
                          </Button>
                        </AlertDialog.Cancel>
                        <AlertDialog.Action
      ...
      <div className="flex flex-col flex-1 py-4 sm:px-2 lg:py-0">
        ...
        {loadingPlaylistData &&
          Array.from({ length: 3 })
            .fill(1)
            .map((item, i) => (
              <VideoCard2 key={i} loading={loadingPlaylistData} />
            ))}
        ...
        <hr
          hidden={!playlist?.data?.videos.length}
