...
import { Button, Dialog, DropdownMenu, Flex, IconButton, Skeleton, Table, Text, TextArea } from '@radix-ui/themes'
...
import { useDeleteTweet, useFetchUserTweets, useUpdateTweet } from '../../lib/queries/tweetQueries'
import { timeAgo } from '../../utils/formatTimeAgo'
import { toIndianDateFormat } from '../../utils/utils'

function TweetsTab() {
  const { user } = useAuth()
  const { data: userTweets, isLoading, isError, error, refetch } = useFetchUserTweets(user?._id, user?._id)
        ...
        <Table.Root>
          ...
          <Table.Body>
            {userTweets?.data.map((tweet) => (
              <Table.Row key={tweet._id} className='bg-[--color-background] transition-all even:bg-[--gray-2] hover:bg-[--color-surface]'>
                <Table.RowHeaderCell
                  className='min-w-[320px] max-w-[320px]'
                >
                  <TweetContentDialog tweet={tweet}>
                    <Text as='p' className='break-words whitespace-pre-wrap line-clamp-3 '>
                      {tweet.content}
                    </Text>
                  </TweetContentDialog>
                </Table.RowHeaderCell>
                <Table.Cell className='text-nowrap'>{toIndianDateFormat(tweet.updatedAt)}</Table.Cell>
...
export function TweetContentDialog({ children, tweet }) {
  ...
  const { mutate: updateTweet, isPending: updatingTweet } = useUpdateTweet(tweet?._id, user?._id)
    ...
    >
      ...
      <Dialog.Content>
        ...
        <Dialog.Description size={'2'} className='break-words whitespace-pre-wrap'>
          {isEditable && <div className='flex-1'>
            ...
            <div className='flex justify-end gap-2 mt-2 '>
              ...
              <Button
                ...
                disabled={!content?.trim() || content?.trim() === tweet?.content || updatingTweet}
          ...
          {!isEditable && <div className='flex flex-col gap-2'>
            <Text
              as='span'
              size={'1'}
              color='gray'
            ...
            <Flex align={'center'} justify={'between'}>
              ...
              >
                <ThumbsUpIcon size={18} strokeWidth={1.25} /> {tweet.likesCount}
              </Text>
              <MoreOptionsMenu tweetData={tweet} setIsEditable={setIsEditable} />
            </Flex>
          </div>}
        </Dialog.Description>
      </Dialog.Content>
...
export function MoreOptionsMenu({ tweetData, setIsEditable }) {
  const { user } = useAuth()
  const { mutateAsync: deleteComment, isPending: deletingComment } = useDeleteTweet(tweetData?._id, user?._id)
    ...
    <>
      ...
      <DropdownMenu.Root>
        ...
        >
          ...
          <DropdownMenu.Item color='red' onClick={() => setOpenConfirmation(true)}>
            <TrashIcon /> Delete
          </DropdownMenu.Item>
        </DropdownMenu.Content>
      </DropdownMenu.Root>
    ...
    <Table.Root>
      ...
      <Table.Body>
        {Array.from({ length: 3 }).map((_, idx) => (
          <Table.Row key={idx}>
            <Table.RowHeaderCell
              className='min-w-[312px] max-w-[312px]'
            >
