...
export default function ReplyCommentCard({
  ...
  const { mutate: updateReply, isPending: isUpdatingReply } =
    useUpdateReplyComment(commentId, user);
  ...
  const { mutate: toggleLike, isPending: isToggleLikePending } =
    useToggleReplyLike(commentId, reply?._id);
        ...
        <>
          <Flex gap={"1"} direction={"column"} flexGrow={"1"}>
            ...
            <div className="flex items-center gap-1 mt-2 text-xs">
              ...
              >
                {isReplyLiked ? <ThumbsUpSolidIcon /> : <ThumbsUp />}
              </IconButton>
              <Text as="span" color="gray" size={"1"}>
                {likesCount}
              </Text>
          ...
          <div>
            {isAuthenticated && user?._id === reply?.owner._id && (
              <>
                <DropdownMenu.Root>
