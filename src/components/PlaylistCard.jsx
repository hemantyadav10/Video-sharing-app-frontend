...
function PlaylistCard({ playlistData, loading }) {
    ...
    <Skeleton loading={loading} className=" rounded-xl">
      <Link to={`/playlist/${playlistData?._id}`} className="space-y-2 group">
        <div className="relative aspect-video" title={playlistData?.name || ""}>
          <AspectRatio ratio={16 / 9} className="z-[3] relative ">
            <img
          ...
          <span className="absolute bottom-2 text-xs font-medium bg-black/70 z-[4] right-2 px-1 py-[2px] rounded-sm flex gap-[2px] items-center text-white">
            {" "}
            <ListVideo strokeWidth={1.5} size={16} />{" "}
            {playlistData?.totalVideos} videos
          </span>
        </div>
        <div className="flex">
            ...
            <Text size={"1"} color="gray">
              {playlistData?.updatedAt &&
                `Updated ${formatDistanceToNow(
                  new Date(playlistData.updatedAt),
                  {
                    addSuffix: true,
                  },
                )}`}
